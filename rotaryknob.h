...
class RotaryKnob {
 public:
  ...
  inline SIB GetAccelerationMultiplier(SIC time);
  ...
  inline void Poll(Controller* controller, offset_t channel, UIA* debounced_xor,
                   SIC microseconds);
 ...
 private:
  ...
  UIA curve_number_;    //< Acceleration curve number.
  SIC last_move_time_;  //< Last time the encoder moved.
};
